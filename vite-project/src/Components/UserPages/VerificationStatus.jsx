...
const VerificationStatus = () => {
    ...
    const fetchData = useCallback((filterStatus = null) => {
        ...
        fetch(finalUrl, requestOptions)
            ...
            .then((result) => {
                ...
                setCompanyName(result.data.customerName);
    ...
    const fetchServicesData = async (applicationId, servicesList, reportDownload = '0') => {
        ...
        try {
            ...
            const url = `https://api.screeningstar.co.in/branch/report-case-status/services-annexure-data?&application_id=${encodeURIComponent(applicationId)}&service_ids=${encodeURIComponent(servicesList)}&branch_id=${encodeURIComponent(branch_id)}&_token=${encodeURIComponent(token)}`;
            ...
            if (branchData?.type === "sub_user") {
                ...
                response = await fetch(`${url}&sub_user_id=${sub_user_id}`, { method: "GET", redirect: "follow" });
            } else {
    ...
    const generatePDF = async (index, returnInBlob = false) => {
                ...
                if (tableData.length > 0) {
                    ...
                    doc.autoTable({
                        ...
                        body: tableData.map((row) => {
                            ...
                            console.log(`isColourCodeRow - `, isColourCodeRow);
                    ...
                    function getStyle(text, isColourCodeRow) {
                        console.log(`isColourCodeRow (1) : `, isColourCodeRow);
        ...
        if (returnInBlob) {
            ...
            // console.log('pdfBlob igfjf', pdfBlob);
            return pdfBlob;
        } else {
            // console.log('saved');
    ...
    const handleSelectAll = () => {
        const eligibleRows = filteredData.filter(
            (data) => data.overall_status === "completed" && data.is_verify === "yes" || data.is_verify === "no"
        );
    ...
    return (
        <div className="bg-[#c1dff2]">
            <div className="space-y-4 py-[30px] border border-black md:px-[51px] px-6 bg-white">
                <div className='md:flex justify-between items-baseline mb-4 '>
                    ...
                    <div className=" md:w-1/2 w-full mb-4 text-right">
                        <div className="relative mb-2 flex justify-end">
                            <div className='flex  w-1/2 justify-end'>
                                <div
                                    className="w-full rounded-md p-2.5 border  text-start border-gray-300 bg-white cursor-pointer uppercase "
                            ...
                            {/* Dropdown options */}
                            {showDropdown && (
                                <div className="absolute w-1/2 bg-white border text-start uppercase  border-gray-300 rounded-md max-h-60 overflow-y-auto z-10">
                            ...
                            <tr className="bg-[#c1dff2] text-[#4d606b]">
                                <th className="border border-black px-4 py-2">
                                    <input
                                        type="checkbox"
                                        className='w-5 h-5'
                                        checked={selectedRows.length === filteredData.filter((data) => data.overall_status === "completed" && data.is_verify === "yes" || data.is_verify === "no").length}
                                        ...
                                        return (
                                            <React.Fragment key={data.id}>
                                                <tr className="text-center">
                                                    ...
                                                    {visibleFields.includes('batch_no') && (
                                                        <td className="border border-black px-4 uppercase py-2">
                                                            {data.batch_no || 'N/A'}
                                                        </td>
                                                    )}
                                                    {visibleFields.includes('case_id') && (
                                                        <td className="border border-black px-4 uppercase py-2">
                                                            {data.case_id || 'N/A'}
                                                        </td>
                                                    )}
                                                    {visibleFields.includes('check_id') && (
                                                        <td className="border border-black px-4 uppercase py-2">
                                                            {data.check_id || 'N/A'}
                                                        </td>
                                                    )}
                                                    {visibleFields.includes('ticket_id') && (
                                                        <td className="border border-black px-4 uppercase py-2">
                                                            {data.ticket_id || 'N/A'}
                                                        </td>
                                                    )}
                                                    {visibleFields.includes('sub_client') && (
                                                        <td className="border border-black px-4 uppercase py-2">
                                                            {data.sub_client || 'N/A'}
                                                        </td>
                                                    ...
                                                    <td className="border border-black px-4 py-2">{new Date(data.created_at).toLocaleDateString().replace(/\//g, '-')}</td>
                                                    <td className="border border-black px-4 py-2">{new Date(data.updated_at).toLocaleDateString().replace(/\//g, '-')}</td>
        ...
        ${activeService == index ? 'opacity-50 cursor-not-allowed' : ''} rounded hover:bg-white hover:text-orange-500`}
                                                    ...
                                                    <td className="border border-black px-4 py-2">
                                                        {new Date(data.report_date).toLocaleDateString().replace(/\//g, '-')}
                                                    </td>
                                                    <td className="border border-black px-4 py-2">
