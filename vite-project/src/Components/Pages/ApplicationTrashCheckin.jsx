...
const ApplicationTrashCheckin = () => {
    ...
    const generatePDF = async (index, returnInBlob = false) => {
        ...
        doc.rect(logoBorderX, logoBorderY, logoBorderWidth, logoBorderHeight); // Custom Logo border
                ...
                if (tableData.length > 0) {
                    ...
                    doc.autoTable({
                        ...
                        body: tableData.map((row) => row.length === 2
                            ? [
                                { content: row[0], styles: { halign: "left", fontStyle: "bold" } }, // Bold for "PARTICULARS"
                                { content: row[1], colSpan: 2, styles: { halign: "left", fontStyle: "normal;" } } // Normal for other columns
                            ]
                            : [
                                { content: row[0], styles: { halign: "left", fontStyle: "bold" } }, // Bold for "PARTICULARS"
                                { content: row[1], styles: { halign: "left", fontStyle: "normal" } }, // Normal for "APPLICANT DETAILS"
                                { content: row[2], styles: { halign: "left", fontStyle: "normal" } } // Normal for "VERIFIED DETAILS"
                            ]
                        ),
                        startY: yPosition,
                        styles: {
                                    ...
                                    if (imageBases) {
                                        for (const [index, image] of imageBases.entries()) {
                                            ...
                                            try {
                                                ...
                                                const padding = 5;
                                                // Full width and height for the border box
                                                ...
                                                let height = (width * image.height) / image.width; // Maintain aspect ratio

                                                // Ensure the image does not exceed the box height
                                                if (height > maxBoxHeight - 2 * padding) {
                                                    height = maxBoxHeight - 2 * padding; // Scale down if necessary
                                                }

                                                // Center the image inside the full box
                                                ...
                                                // Move yPosition for the next content
...
report.For any clarifications you can mail us at `;
        ...
        const anchorText = modifiedNames[0] || "compliance@screeningstar.com";
        ...
        const firstLink = (modifiedNames && modifiedNames.length > 0)
            ? `mailto:${modifiedNames[0]}`
            : "mailto:compliance@screeningstar.com";
        doc.textWithLink(anchorText, part2X, currentY - lineHeight, { url: firstLink });


        // Company Details Section
        let companyDetailsY = currentY + disclaimerTextTopMargin - 2;

        // Company Name Section
        doc.setFont('TimesNewRomanBold');
        doc.setFontSize(9); // Reduced font size for the company name
        doc.setTextColor(0, 0, 0); // Soft blue color
        const companyText = applicationInfo.customer_name || "ScreeningStar Solutions Private Limited";
        const leftMargin = 10; // Left alignment margin
        doc.text(companyText, leftMargin, companyDetailsY - 2);

        companyDetailsY -= 4; // Add spacing below the company name
        doc.setFontSize(9); // Reduced font size for the address section
        // doc.setFont("helvetica", "normal");
        ...
        addressToUse.forEach((line) => {
            ...
            companyDetailsY += 7; // Adjust spacing between lines
        });
    ...
    const handleSelectAll = () => {
        const eligibleRows = filteredData.filter(
            (data) => data.overall_status === "completed" && data.is_verify === "yes"
        );

        const allIds = eligibleRows.map((data) => data.id);
        const limitedSelection = allIds.slice(0, 25); // Limit to 25 checkboxes

        setSelectedRows(
            limitedSelection.length + selectedRows.length > 25
                ? selectedRows
                : limitedSelection
        );
    };
    ...
    const handleBulkDownload = async () => {
        ...
        let firstApplicationInfo = null; // To generate a dynamic ZIP name

        for (const id of selectedRows) {
            try {
                const index = data.findIndex((application) => application.id === id);
                if (index === -1) {
                    continue;
                }
                ...
                if (!firstApplicationInfo) {
                    firstApplicationInfo = applicationInfo; // Store the first record for ZIP name
                }

                console.log(`Data index: ${index}, Data:`, applicationInfo);

                const pdfBlob = await generatePDF(index, true);
   ...
   const formatDate = (date) => {
    ...
    const handleApplicationDelete = (id) => {
        Swal.fire({
            title: "Are you sure?",
            text: "Delete this application permanently? This action cannot be undone!",
            icon: "warning",
    ...
    const handleGoBack = () => {
        navigate('/admin-TrashApplications');  // Navigate to the /adminjkd path
    };

    const handleApplicationRestore = (id) => {
        Swal.fire({
            title: "Are you sure?",
            text: "Do you really want to Restore this application?",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                ...
                fetch(url, requestOptions)
                    ...
                    .catch((error) => {
                        ...
                        // Extract or set dynamic error message based on the error
                        const errorMessage = error.message.includes('HTTP error')
                            ? `Failed to Restore application. Server returned status ${error.message.split(': ')[1]}`
                ...
                // If user clicks "No"
                Swal.fire('Cancelled', 'The application was not Restored.', 'info');
            }
        });
    };
    ...
    console.log('modifired name', modifiedNames);
    return (
                    ...
                    <div className=" text-left">
                        ...
                        >
                            ...
                            {selectedRows.length > 0 && (
                                ...
                                >
                                    {isBulkDownloading ? 'Downloading...' : `Bulk Download (${selectedRows.length})`}
                                </button>
...
export default ApplicationTrashCheckin;
