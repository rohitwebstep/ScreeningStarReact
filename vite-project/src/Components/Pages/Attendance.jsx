...
const Attendance = () => {
    ...
    const exportToExcel = () => {
        const exportData = paginatedData.map((row, index) => ({
            ...
            "Login Time": formatDate(row.first_check_in_time),
            "Logout Time": formatDate(row.last_check_out_time)
        }));
                                        ...
                                        <tr className="text-center">
                                            ...
                                            <td className="border border-black text-center capitalize px-4 py-2">
                                                <div className="flex justify-center items-center">
                                                    <img src={row.profile_picture || Default} alt={row.admin_name} className="w-10 h-10 rounded-full" />
                                                </div>
                                            </td>
  ...
  {formatDate(row.breaks?.find(b => b.type === 'login')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'tea break in-1')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'tea break out-1')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'lunch break in')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'lunch break out')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'tea break in-2')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'tea break out-2')?.time)}
</td>
<td className="border border-black capitalize px-4 py-2">
  {formatDate(row.breaks?.find(b => b.type === 'logout')?.time)}
</td>
                                            ...
                                            {/* <td className="border border-black px-4 py-2">
                                                <button
                                                    onClick={() => handleView(row.admin_id)}
                                                    className={`${expandedRow === row.admin_id
                                                            ? "bg-red-500 hover:bg-red-700"
                                                            : "bg-blue-500 hover:bg-blue-700"
                                                        } text-white hover:scale-105 font-bold transition duration-200 px-4 py-2 rounded`}
                                                >
                                                    {expandedRow === row.admin_id ? "Hide" : "View"}
                                                </button>
                                        ...
                                        {expandedRow === row.admin_id && (
                                            <tr className="bg-gray-100">
                                                <td colSpan="7" className="border border-black px-4 py-2">
                                                    {row.check_in_outs && row.check_in_outs.length > 0 ? (
                                                        <div className="space-y-2">
                                                            {row.check_in_outs.map((entry, idx) => (
                                                                <div key={idx} className="flex justify-between border w-10/12 m-auto p-2 rounded bg-white shadow-sm">
                                                                    <span className="font-semibold text-blue-700 capitalize">Status:</span>{' '}
                                                                    <span
                                                                        className={`font-semibold ${entry.status.toLowerCase() === 'check-in'
                                                                            ? 'text-green-600'
                                                                    ...
                                                                    >
                                                                        {entry.status.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())}
                                                                    </span>
                                                                    <span className="font-semibold text-blue-700">Time:</span> {formatDate(entry.time)}
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ) : (
                                                        <div className="text-red-500">No check-in/out data available.</div>
                                                    )}
                                                </td>
                                            </tr>
                                        )}
