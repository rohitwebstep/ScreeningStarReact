...
const AdminCandidateManager = () => {
    ...
    const fetchData = useCallback((filterStatus = null) => {
        ...
        const baseUrl = `https://api.screeningstar.co.in/candidate-master-tracker/list`;
        ...
        if (filterStatus) {
            queryParams.append('filter_status', filterStatus); // Add filter status to the URL if present
        }
    ...
    const handleCheckIn = (main_id) => {
        ...
        fetch(`https://api.screeningstar.co.in/candidate-master-tracker/branch-list-by-customer?customer_id=${main_id}&admin_id=${adminId}&_token=${token}`, requestOptions)
            .then((response) => {
                if (!response.ok) {
    ...
    const handleCheckInGo = (branch_id, main_id, branchName) => {
        navigate(`/admin-CandidateCheckin?clientId=${main_id}&branchId=${branch_id}&BranchName${branchName}`);
    };
    const Loader = () => (
    ...
    const filteredDrowpownData = filterData.filter((item) =>
            ...
            <div className="bg-white border border-black md:p-12 p-6 w-full mx-auto">
                ...
                <div className='overflow-x-scroll'>
                    <table className="min-w-full border-collapse border border-black rounded-lg">
                        <thead>
                            <tr className="bg-[#c1dff2] whitespace-nowrap text-left text-[#4d606b]">
                                                    ...
                                                    <td className="border border-black px-4 py-2 text-center">
                                                        <div className='md:block flex whitespace-nowrap'>
                                                            {item.application_count <= item.head_branch_applications_count ? (
                                                                ...
                                                                <button
                                                                    className="px-4 uppercase py-2 text-white rounded-md font-bold bg-green-500 hover:bg-green-600 hover:scale-105 transition-transform duration-300 ease-in-out transform"
