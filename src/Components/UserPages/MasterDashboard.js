...
const MasterDashboard = () => {
  ...
  const renderApplicationStats = () => {
    return Object.keys(branchData).map((category) => {
      ...
      return (
        <div key={category} className={`flex items-center justify-between w-[32%] p-4 ${backgroundColor} rounded shadow`}>
          <div className="flex flex-col">
            <p className="text-sm font-semibold text-black">{category.toUpperCase()}</p>
            <span className="text-xl font-bold text-black">{count}</span>
          </div>
          <div>
            <i className={`fa-solid fa-dollar-sign text-2xl text-blue-600`}></i>
          </div>
        </div>
      );
    });
          ...
          <tbody>
            {currentEntries.map((application, index) => (
              <tr key={index}>
                ...
                <td className="border px-4 py-2">{application.application_name}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {loading ? (
        ...
        <>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 bg-[#c1dff2] gap p-5 gap-5">
