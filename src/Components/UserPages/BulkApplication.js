...
const BulkApplication = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.clientSpocId) newErrors.clientSpocId = "SPOC name is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      setFormData({ organizationName: customer.name, clientSpocId: "", remarks: "", files: [] || '' });
      ...
      if (branchData) {
        ...
        customerLogoFormData.append('client_spoc_id', formData.clientSpocId);
          ...
          <div className='flex space-x-4'>
            <div className="w-2/5">
              ...
              <div className="w-full">
                ...
                <select
                  name="clientSpocId"
                  value={formData.clientSpocId}
                  onChange={handleChange}
                  className={`w-full m-auto p-3 mb-[20px] border ${errors.clientSpocId ? "border-red-500" : "border-gray-300"} rounded-md`}
                >
                  <option value="" >Select SPOC Name</option>
                  {customer.client_spoc_details && customer.client_spoc_details.length > 0 ? (
                    customer.client_spoc_details.map((spoc) => (
                    ...
                    <option value="" disabled>
                      No SPOC details available
                    </option>
                  )}
                </select>

                {errors.clientSpocId && (
                  <p className="text-red-500 text-sm">{errors.clientSpocId}</p>
                )}
              </div>
                      ...
                      displayedData.map((entry, index) => (
                        <tr key={entry.id} className="text-center">
                          ...
                          <td className="border border-black px-4 py-2">
                            {
                              customer.client_spoc_details && customer.client_spoc_details.length > 0
                                ? customer.client_spoc_details.find(spoc => spoc.id == entry.client_spoc_id)
                                  ? customer.client_spoc_details.find(spoc => spoc.id == entry.client_spoc_id).name
                                  : 'No SPOC details available'
                                : 'No SPOC details available'
                            }
                          </td>
