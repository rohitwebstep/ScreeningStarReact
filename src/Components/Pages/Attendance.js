            ...
            <tbody>
              {loading ? (
                ...
                paginatedData.map((row, index) => {
                  ...
                  const isLeave = row.status?.toLowerCase() === "on leave";
                  const isPresent = row.status?.toLowerCase() === "present";


                  const empAttendanceCountMap = {};

                  paginatedData.forEach((row) => {
                    const key = row.emp_id;
                    if (!empAttendanceCountMap[key]) {
                      empAttendanceCountMap[key] = {
                        leaveCount: 0,
                        presentCount: 0,
                    ...
                    const hasLoginOrLogout = row.first_login_time || row.last_logout_time;

                    if (hasLoginOrLogout) {
                      empAttendanceCountMap[key].presentCount += 1;
                    } else {
                      empAttendanceCountMap[key].leaveCount += 1;
                    }
                  });

                  paginatedData.forEach((emp) => {
                    const counts = empAttendanceCountMap[emp.emp_id];
                    emp.leaveCount = counts.leaveCount;
                    emp.presentCount = counts.presentCount;
                    emp.fromDate = counts.fromDate;
                    emp.toDate = counts.toDate;
                  });


                  return (
                    <React.Fragment key={index}>
                      {orderedBreakTypes.map((label, i) => (
                        <tr key={`${index}-${i}`} className="text-center">
                          {i === 0 && (
                            <>
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                                {index + 1 + (currentPage - 1) * itemsPerPage}
                              </td>
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                                {row.emp_id}
                              </td>
                              <td rowSpan={orderedBreakTypes.length} className="border border-black capitalize">
                                {row.admin_name}
                              </td>
                            </>
                          )}

                          <td className="border border-black capitalize">{label}</td>

                          {Array.from({ length: 31 }, (_, d) => {
                            const isCurrentDay = d + 1 === new Date(row.date).getDate();
                            const key = label.toLowerCase();
                            let value = "";

                            if (label === "LOGIN" && isCurrentDay)
                              value = row.first_login_time;
                            else if (label === "LOGOUT" && isCurrentDay)
                              value = row.last_logout_time;
                            else if (label !== "LOGIN" && label !== "LOGOUT" && isCurrentDay)
                              value = breakTimes?.[key];
                          ...
                          {i === 0 && (
                            <>
                              {/* ✅ Corrected LEAVE and PRESENT counts */}
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                                {row.leaveCount}
                              </td>
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                              ...
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                                {row.fromDate ? formatDate2(row.fromDate) : ""}
                              </td>
                              <td rowSpan={orderedBreakTypes.length} className="border border-black">
                                {row.toDate ? formatDate2(row.toDate) : ""}
                              </td>
                            </>
                          )}
                        </tr>
                              ...
                              <div className="space-y-2">
                                {row.check_in_outs.map((entry, idx) => (
                                  ...
                                  >
                                    ...
                                    <span
                                      className={`font-semibold ${entry.status.toLowerCase() ===
                                        "check-in"
                                        ? "text-green-600"
