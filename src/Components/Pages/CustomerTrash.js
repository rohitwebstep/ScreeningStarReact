...
const CustomerTrash = () => {
  ...
  const handleDelete = useCallback((id) => {
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: `Delete this Customer permanently? This action cannot be undone!`,
      icon: 'warning',
      showCancelButton: true,
  ...
  const handleRestore = useCallback((id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setIsRestoreLoading(true); // Start the loading state
        setActiveId(id);
        // Get admin ID and token from localStorage
        const admin_id = JSON.parse(localStorage.getItem("admin"))?.id;
        const storedToken = localStorage.getItem("_token");
        ...
        fetch(`https://api.screeningstar.co.in/trashed/customer/restore/?id=${id}&admin_id=${admin_id}&_token=${storedToken}`, requestOptions)
          ...
          .then((result) => {
            ...
            fetchActiveAccounts(); // Refresh the list of active accounts
            Swal.fire('Deleted!', 'The customer has been Restored.', 'success');
          })
          .catch((error) => {
            console.error(error);
            Swal.fire('Error!', 'There was an issue Restoring the customer.', 'error');
          })
          .finally(() => {
