...
const GenerateInvoice = () => {
  ...
  function addFooter(doc) {
    const totalPages = doc.internal.getNumberOfPages(); // Get total page count
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      const footerYPosition = pageHeight - 10; // Position 10 units from the bottom
      ...
      doc.text(footerText, pageWidth / 2, footerYPosition, { align: "center" });
    }
  }
  ...
  const generatePdf = (serviceNames, serviceInfo, overallServiceAmount, cgst, sgst, totalTax, customer, applications, totalAmounts, companyInfo) => {
    ...
    const companyName = companyInfo?.name || 'Naaa/A';
    ...
    doc.text(companyName, textX, textY, { align: "center" });
    ...
    if (formattedLine1) {
      doc.text(formattedLine1, textX, textY + 5, { align: "center" });
    }
    if (formattedLine2) {
    ...
    if (companyInfo?.phone_number && companyInfo?.email) {
      doc.text(
        `PH: +91-${companyInfo.phone_number} | Email: ${companyInfo.email}`,
        textX,
        textY + 20,
        { align: "center" }
      );
    ...
    if (companyInfo?.website) {
      doc.text(`Web: ${companyInfo.website}`, textX, textY + 25, { align: "center" });
    }
    ...
    const qrCodeBase64 =
      "https://cdn.pixabay.com/photo/2013/07/12/14/45/qr-code-148732_640.png"; // Replace with your base64-encoded image or a URL
    ...
    doc.addImage(
      qrCodeBase64,
      "PNG",
      thirdColumnX + (thirdColumnWidth - 60) / 2, // Center the QR code horizontally
      topMargin + (columnHeight - 40) / 2, // Center the QR code vertically
      30, // Set width to 60px
      30  // Set height to 60px
    );
    ...
    doc.setFillColor("#284873"); // Light gray
    doc.rect(leftMargin, taxInvoiceY, pageWidth - 2 * leftMargin, taxInvoiceHeight, "F");
    ...
    const rowY = taxInvoiceY + taxInvoiceHeight + 10; // Position below the "TAX INVOICE" background
    ...
    doc.rect(column1X, rowY, columnWidth, rowHeight); // Add border
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(0, 0, 0); // Black text
    doc.text("Invoice Details", column1X + padding, rowY + rowHeight / 2 + 1); // Align left by adjusting the x-coordinate

    // Column 2 (Details of Service Recipient)
    doc.rect(column2X, rowY, columnWidth, rowHeight); // Add border
    doc.text("Details of Service Recipient", column2X + padding, rowY + rowHeight / 2 + 1); // Align left by adjusting the x-coordinate
    ...
    doc.text("Bill To:", column1XNext + padding, labelY); // Bill To label
    doc.text(customer.name || "N/A", column1XNext + 20, labelY); // Value for Bill To, placed beside the label
    labelY += 7; // Move to next row with 10px gap

    doc.text("Attention:", column1XNext + padding, labelY); // Attention label
    doc.text(customer.address || "N/A", column1XNext + 20, labelY); // Value for Attention, placed beside the label

    // Column 2 (Client GST NO, Invoice Number, etc.)
    ...
    doc.text("Client GST NO:", column2XNext + padding, labelYSecondColumn); // Client GST NO label
    doc.text(customer.gst_number || "N/A", column2XNext + 40, labelYSecondColumn); // Value for Client GST NO
    ...
    doc.text(customer.state || "N/A", column2XNext + 40, labelYSecondColumn); // Value for State
    labelYSecondColumn += 7; // Move to next row with 5px gap

    doc.text("Date Of Service:", column2XNext + padding, labelYSecondColumn); // Date Of Service label
    doc.text(formData.invoice_date, column2XNext + 40, labelYSecondColumn); // Value for Date Of Service
    labelYSecondColumn += 7; // Move to next row with 5px gap
    ...
    const tableBody2 = applications.flatMap((applicationArr) => {
      return applicationArr.applications.map((app) => {
        ...
        const appTotalTax = (
          parseFloat(appTotalPricing) *
          (overAllTotalTaxPercentage / 100)
        ).toFixed(2);
        ...
        result = {
          ...
          qty: app.created_at || "NIL",
          rate: app.name || "NIL",
          ...servicePriceDetails,
          additionalFee: additionalFeeSum || "0.00",
          taxableAmount: servicePricesSum || "0.00",
          reportDate: app.report_date || "Nil",
        };
    ...
    const serviceTableBody = overallServicePricingByService.map((item) => {
      ...
      totalSubPrice += parseFloat(price);
      const totalPrice = parseFloat(item.totalPrice).toFixed(2);
      const totalAddFee = parseFloat(item.totalAddFee).toFixed(2);
      const appTotalPriceWithTax = (
        parseFloat(appTotalTax) + parseFloat(item.totalPrice)
      ).toFixed(2);

      totalServiceQty += item.qty;
      // Return an array for the table row, with the correct formatting
      return {
    ...
    serviceTableBody.push({
      ...
      rate: String(totalSubPrice.toFixed(2)),
      additionalFee: String(overallApplicationsAdditionalFeeSum.toFixed(2)),
      taxableAmount: String(overallApplicationsTotalTax.toFixed(2)),
    });
    ...
    doc.autoTable({
      ...
      headStyles: {
        ...
        lineColor: "#000", // Border color for header
        lineWidth: 0.4, // Border width for header
      },
      bodyStyles: {
    ...
    const overAllCgstTax = (totalSubPrice * (overAllCgstPercentage / 100));
    const overAllSgstTax = (totalSubPrice * (overAllSgstPercentage / 100));

    const overAllIGSTTax = overAllCgstTax + overAllSgstTax;

    const overAllTotalTax = overAllIGSTTax + overAllCgstTax + overAllSgstTax;
    ...
    const taxDetails = [
      { label: `CGST ${overAllCgstPercentage}%`, value: String(overAllCgstTax) },
      { label: `SGST ${overAllSgstPercentage}%`, value: String(overAllSgstTax) },
      { label: `IGST ${overAllTotalTaxPercentage}%`, value: String(overAllIGSTTax) },
      { label: 'Total GST', value: String(overAllTotalTax) },
      { label: 'Total Tax Amount (Round off)', value: String(overAllTotalTax) },
    ];
    ...
    doc.setFontSize(10);
    doc.rect(leftMargin, tableStartYNew, bankDetailsWidth, 12); // Bank Details Header
    doc.text("SCREENINGSTAR BANK ACCOUNT AND TAX DETAILS", leftMargin + 5, tableStartYNew + 7);
    // Defines the height of the header row

    doc.rect(leftMargin + bankDetailsWidth, tableStartYNew, taxDetailsWidth, 12); // Tax Details Header
    ...
    // Label Column (60% width)
    const labelXPosition = startX + (labelWidths / 2) - (doc.getTextDimensions("TOTAL AMOUNT BEFORE TAX").w / 2); // Center the label text in the column
    doc.rect(startX, tableStartYNew, labelWidths, headerHeight); // Draw the label column rectangle
    doc.text("TOTAL AMOUNT BEFORE TAX", labelXPosition, tableStartYNew + 7); // Label text centered
    ...
    // Convert totalSubPrice to string, ensuring two decimal places if needed
    doc.text(totalSubPrice.toFixed(2), valueXPosition, tableStartYNew + 7);
    ...
    for (let i = 0; i < maxRows; i++) {
      const bankItem = bankDetails[i] || { label: "", value: "" }; // Fallback for empty rows
      const taxItem = taxDetails[i] || { label: "", value: "" }; // Fallback for empty rows

      const rowHeight = 7; // Set a fixed height for all rows


      // Bank Details
      doc.setFont("helvetica", "normal");
      doc.rect(leftMargin, currentY, bankDetailsWidth / 2, rowHeight); // Label column for Bank Details
      doc.text(bankItem.label, leftMargin + 5, currentY + 5);

      doc.rect(leftMargin + bankDetailsWidth / 2, currentY, bankDetailsWidth / 2, rowHeight); // Value column for Bank Details
      doc.text(bankItem.value, leftMargin + bankDetailsWidth / 2 + 5, currentY + 5);

      // Tax Details
      // Calculate widths for the Tax Details columns
      const taxLabelWidth = taxDetailsWidth * 0.6; // 60% for the label
      const taxValueWidth = taxDetailsWidth * 0.4; // 40% for the value

      // Center text in the label column (60% width)
      const labelXPosition = leftMargin + bankDetailsWidth + (taxLabelWidth / 2) - (doc.getTextDimensions(taxItem.label).w / 2); // Calculate the center of the label column
      doc.rect(leftMargin + bankDetailsWidth, currentY, taxLabelWidth, rowHeight); // Label column
      doc.text(taxItem.label, labelXPosition, currentY + 5); // Label text centered

      // Center text in the value column (40% width)
      const valueXPosition = leftMargin + bankDetailsWidth + taxLabelWidth + (taxValueWidth / 2) - (doc.getTextDimensions(taxItem.value).w / 2); // Calculate the center of the value column
      doc.rect(leftMargin + bankDetailsWidth + taxLabelWidth, currentY, taxValueWidth, rowHeight); // Value column
      doc.text(taxItem.value, valueXPosition, currentY + 5); // Value text centered
    ...
    doc.setFont("helvetica", "bold");
    doc.rect(leftMargin, currentY, bankDetailsWidth + taxDetailsWidth, lastRowHeight); // Label column
    doc.text(`Total Tax Amount : ${overAllAmountWithTax}`, leftMargin + 5, currentY + 7);

    currentY += lastRowHeight; // Update Y position

    // Create header dynamically for serviceCodes
    const serviceCodeHeaders = serviceCodes.map(code => ({
      header: `${code}`, // You can change the label as needed
      dataKey: `serviceCode${code}`   // Use a unique dataKey for each service code
    }));
    ...
    doc.autoTable({
      ...
      bodyStyles: {
        halign: 'center', // Center align text in body cells
        lineColor: "#000", // Border color for body cells
        lineWidth: 0.4,
        textColor: "#000", // Border width for body cells
      },
      margin: { top: currentY + 10, left: leftMargin, right: leftMargin }, // Set Y position below the previous section
      tableWidth: 'auto',
      tableLineColor: "#000",
      tableLineWidth: 0.4
    });
    ...
    doc.setFontSize(9);
    doc.text("Authorized Signatory", pageWidth / 2, stampYPosition + 40, { align: "center" });
    ...
    doc.save(`${clientCode || "N/A"}-${formData.invoice_date}`);
        ...
        <div className="bg-white text-left  p-12   w-full mx-auto">
          <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
            <div>
              ...
              <SelectSearch
                options={options.length === 0 ? [{ value: '0', name: 'You have no data' }] : options}
                value={formData.client_code}
                name="client_code"
                placeholder={options.length === 0 ? 'You have no data' : 'Choose your language'}
                onChange={(value) => handleChange({ target: { name: 'client_code', value }})}
                search
                disabled={options.length === 0} // Disable if no options are available
              />
