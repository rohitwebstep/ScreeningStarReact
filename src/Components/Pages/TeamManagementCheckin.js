...
const TeamManagementCheckin = () => {
    ...
    const fetchData = useCallback((filterStatus = null) => {
        ...
        fetch(finalUrl, requestOptions)
            .then((response) => {
                ...
                return response.json().catch(() => ({})); // Handle cases where response is not JSON
            })
            .then((result) => {
    ...
    const fetchServicesData = async (applicationId, servicesList, reportDownload = '0') => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('vfilteredResults', filteredResults);
    ...
    function addFooter(doc, index) {
        ...
        doc.line(margin, footerYPosition - footerHeight + 15, pageWidth - margin, footerYPosition - footerHeight + 15);

        // **Company Information (Centered)**
        doc.setFont('TimesNewRomanBold');
        doc.setFontSize(12);
        doc.setTextColor(12, 55, 98); // RGB values for #0c3762

        const companyText = applicationInfo.customer_name || "Screeningstar Solutions Private. Limited.";
        doc.text(companyText, pageWidth / 2, footerYPosition + 4, { align: 'center' });

        // **Address & Contact Information (Centered)**
        doc.setFontSize(9);
        ...
        const contactDetails = [
            "M: +91 9980004953 E: compliance@screeningstar.com, W: www.screeningstar.com"
        ];

        let textYPosition = footerYPosition + 10;

        const printCenteredText = (lines, color) => {
            doc.setTextColor(color.r, color.g, color.b);
            lines.forEach((line) => {
                doc.text(line, pageWidth / 2, textYPosition, { align: 'center' });
                textYPosition += 6;
            });
        };
        ...
        if (customAddress && customAddress.length > 0) {
            ...
            printCenteredText(addressLines, { r: 0, g: 0, b: 0 }); // Black for address
            printCenteredText(contactDetails, { r: 0, g: 0, b: 255 }); // Blue for contact details
        }

        // **Page Number (Right-Aligned & Black Color)**
        const pageCount = doc.internal.getNumberOfPages();
        const currentPage = doc.internal.getCurrentPageInfo().pageNumber;
        const pageNumberText = `Page ${currentPage} / ${pageCount}`;
        doc.setFont('TimesNewRomanBold');
        doc.setTextColor(0, 0, 0); // Black color
        doc.text(pageNumberText, pageWidth - margin, footerYPosition + 4, { align: 'right' });
    }
...
console.log('applicationInfo',applicationInfo)
        ...
        if (returnInBlob) {
            ...
            console.log('applicationInfo-----', applicationInfo)
            doc.save(
                `${applicationInfo.application_id || 'NA'}-${applicationInfo.name || 'NA'}-${applicationInfo.employee_id || 'NA'}-${applicationInfo.report_type === 'interim_report' ? 'INTERIM_REPORT' : applicationInfo.report_type === 'final_report'
                    ? 'FINAL_REPORT'
                    : 'UNKNOWN_REPORT'
                }`
            );
    ...
    const handleUpload = (applicationId, branchid) => {
        navigate(`/admin-TeamManagementGenerateReport?applicationId=${applicationId}&branchid=${branchid}&clientId=${clientId}`);
    };
    ...
    return (
        <div className="bg-[#c1dff2]">
            <h2 className="md:text-2xl text-xl font-bold py-3 text-left text-[#4d606b] px-3 border">
                TEAM CHECKIN - {branchName === companyName ? branchName : `${branchName} (${companyName})`}
            </h2>
                    ...
                    <div className=" md:w-1/2 text-right">
                        <div className="text-left flex md:justify-end">
                            <div className="relative w-1/2 mb-2">
                                {/* Dropdown button */}
                                <div
                                    className="w-full rounded-md  uppercase  p-2.5 border border-gray-300 bg-white cursor-pointer"
                                ...
                                {/* Dropdown options */}
                                {showDropdown && (
                                    <div className="absolute w-full bg-white border border-gray-300 uppercase  rounded-md max-h-60 overflow-y-auto z-10">
...
export default TeamManagementCheckin;
